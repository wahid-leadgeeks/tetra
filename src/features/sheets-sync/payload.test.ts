...
describe("computeSheetBreakTimes", () => {
  ...
  it("emits full day cells including attendance, breaks, out, dailyTotal, and workTotal", () => {
    const summary = makeSummary();
    ...
    expect(a1s).toContain(`${NOTES_MAPPING.clockInColumn}${ROW}`);
    expect(a1s).toContain(`${NOTES_MAPPING.clockOutColumn}${ROW}`);
    expect(a1s).toContain(`${NOTES_MAPPING.breakStartColumn}${ROW}`);
    expect(a1s).toContain(`${NOTES_MAPPING.breakEndColumn}${ROW}`);
    expect(a1s).toContain(`${NOTES_MAPPING.dailyTotalColumn}${ROW}`);
